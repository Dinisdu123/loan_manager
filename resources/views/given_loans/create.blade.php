...
@section('content')
    <div class="container mx-auto px-4 py-6 bg-gradient-to-br from-indigo-50 to-purple-100 min-h-screen animate-on-load">
        ...
        <div class="flex justify-between items-center mb-6 animate__animated animate__fadeIn animate__fastest">
            ...
            <a href="{{ route('given_loans.index') }}"
               class="btn-custom bg-gradient-to-r from-gray-500 to-gray-600 text-white px-6 py-3 rounded-lg shadow-md hover:from-gray-600 hover:to-gray-700 flex items-center">
                <i class="fas fa-arrow-left mr-2"></i>Back to Given Loans
            </a>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 max-w-lg mx-auto card-hover bg-gradient-to-br from-white to-gray-50 border border-indigo-200 hover:shadow-xl transition-all duration-300 animate__animated animate__fadeInUp animate__fastest animate__delay-200ms">
            <form method="POST" action="{{ route('given_loans.store') }}">
                @csrf
                <div class="mb-4">
                    <label for="center_id" class="block text-sm font-medium text-indigo-800">Select Center</label>
                ...
                <div class="mb-4">
                    <label for="user_id" class="block text-sm font-medium text-indigo-800">Select User</label>
                ...
                <div class="mb-4">
                    <label for="amount" class="block text-sm font-medium text-indigo-800">Amount</label>
                ...
                <div class="mb-4">
                    <label for="interest_rate" class="block text-sm font-medium text-indigo-800">Interest Rate (% per month)</label>
                    <input type="number" name="interest_rate" id="interest_rate" step="0.01" value="{{ old('interest_rate') }}"
                           class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 text-sm py-2 px-3"
                           required>
                    @error('interest_rate')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="mb-4">
                    <label for="duration_months" class="block text-sm font-medium text-indigo-800">Duration (Months)</label>
                ...
                <div class="mb-4">
                    <label for="start_date" class="block text-sm font-medium text-indigo-800">Start Date</label>
                ...
                <div class="flex justify-end space-x-3">
                    ...
                    <button type="submit"
                        ...
                        <i class="fas fa-check mr-2"></i>Assign Loan
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        // Restore selected user if validation fails
        // @if (old('center_id'))
        //     document.getElementById('center_id').value = "{{ old('center_id') }}";
        //     updateUsers();
        //     @if (old('user_id'))
        //         setTimeout(() => {
        //             document.getElementById('user_id').value = "{{ old('user_id') }}";
        //         }, 500);
        //     @endif
        // @endif
    </script>
